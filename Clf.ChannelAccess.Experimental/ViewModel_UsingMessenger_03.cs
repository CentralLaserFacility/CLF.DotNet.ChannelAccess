//
// ViewModel_UsingMessenger_03.cs
//
...
namespace Clf.ChannelAccess.Experimental.Messenger_03
{

  //
  // The Messenger matches the *exact* type of the published message.
  //
  // If the 'Send' publishes a message as a concrete subclass, the recipient
  // has to be coded to explicitly handle that specific subclass message type,
  // as illustrated below (in code that's commented out).
  //
  // Best scheme is to have the Send publish the message explicitly as a base class,
  // and define the Recipient as a handler for messages of that base class.
  // Then the 'Receive' gets invoked whatever concrete 'message' subclass is sent.
  //

  public class ChannelEventsHandler 
  : CommunityToolkit.Mvvm.Messaging.IRecipient<Clf.ChannelAccess.StateChangedMessage>
  // , CommunityToolkit.Mvvm.Messaging.IRecipient<Clf.ChannelAccess.ConnectionStatusChangedMessage>
  // , CommunityToolkit.Mvvm.Messaging.IRecipient<Clf.ChannelAccess.ValueChangedMessage>
  {
    ...
    public ChannelEventsHandler ( )
    {
      ...
      // CommunityToolkit.Mvvm.Messaging.WeakReferenceMessenger.Default.Register<Clf.ChannelAccess.ConnectionStatusChangedMessage>(
      //   this
      // ) ;
      // CommunityToolkit.Mvvm.Messaging.WeakReferenceMessenger.Default.Register<Clf.ChannelAccess.ValueChangedMessage>(
      //   this
      // ) ;
    }
    ...
    // void CommunityToolkit.Mvvm.Messaging.IRecipient<ConnectionStatusChangedMessage>.Receive ( ConnectionStatusChangedMessage message )
    // {
    //   (this as CommunityToolkit.Mvvm.Messaging.IRecipient<StateChangedMessage>).Receive(
    //     message as StateChangedMessage
    //   ) ;
    // }
    // void CommunityToolkit.Mvvm.Messaging.IRecipient<ValueChangedMessage>.Receive ( ValueChangedMessage message )
    // {
    //   (this as CommunityToolkit.Mvvm.Messaging.IRecipient<StateChangedMessage>).Receive(
    //     message as StateChangedMessage
    //   ) ;
  ...
  public class ViewModel_UsingMessenger_Base
  {
  ...
  public class ViewModel_UsingMessenger : ViewModel_UsingMessenger_Base, System.IDisposable
  {
    ...
    public async Task<bool> TryFinishInitialisationAsync_OLD ( )
    {
      // Here we wait a while for all our channels to report that they've
      // successfully connected. Once that's happened, we can query any
      // of their values etc and also set up event handlers that will 
  ...
  public class Test_ViewModel_UsingMessenger_03
  {
